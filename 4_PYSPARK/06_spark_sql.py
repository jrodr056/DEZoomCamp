...
dfYellowGreen.write.parquet(output+'data/report/revenue/total', mode='overwrite')

dfZones = spark.read.option("header","true").csv('data/taxi+_zone_lookup.csv')

dfZones.write.parquet(output+"data/zones/",mode='overwrite')

dfResult = dfYellowGreen.join(dfZones, dfYellowGreen.zone == dfZones.LocationID)

dfResult.show()

dfResult.drop('LocationID','zone').write.parquet('tmp/revenueZones',mode='overwrite')
